...
export default function AdminDashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-washi px-4 py-6 md:px-6">
      <div className="mx-auto w-full max-w-5xl space-y-6">
        <header className="space-y-2">
          <h1 className="font-serif text-4xl">Dashboard de Conteudo</h1>
          {status && <p className="text-sm text-sumi/80">{status}</p>}
        </header>

        <Card>
          <CardHeader>
            <CardTitle className="font-serif text-2xl">Visibilidade das secoes</CardTitle>
            <CardDescription>Ative ou desative as secoes da landing page.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3">
            {visibility.map((v) => (
              <div key={v.section} className="flex items-center justify-between rounded-md border border-[#e7dece] px-3 py-2">
        ...
        {Object.entries(content.sections).map(([section, fields]) => (
          <Card key={section}>
            <CardHeader>
              <CardTitle className="font-serif text-2xl capitalize">{section.replaceAll("_", " ")}</CardTitle>
